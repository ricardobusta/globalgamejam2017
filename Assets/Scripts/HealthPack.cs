...
public class HealthPack : MonoBehaviour {
  ...
  void OnTriggerEnter(Collider other) {
    ...
    if (other.CompareTag(PLANET_TAG)) {
      GameManager.Instance().soundManager.playEnemyHit();
      GameManager.Instance().explosion.playExplosion(transform.position);
      Destroy(gameObject);
    }
    ...
    // Increases HP
      ...
      if (GameManager.Instance().playerHealth < GameManager.Instance().playerMaxHealth) {
        ...
        GameManager.Instance().soundManager.powerup.Play();
      }
